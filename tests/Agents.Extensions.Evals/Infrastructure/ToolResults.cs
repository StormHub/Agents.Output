...
/// <remarks>
/// <para>
/// This is the grounding truth for the whole suite. <c>UseFunctionInvocation</c> runs the tool
/// loop inside <see cref="IChatClient.GetResponseAsync"/> and appends every intermediate message
/// — the assistant's tool call, the tool's result, the final answer — to
...
internal static class ToolResults
{
    ...
    /// <summary>
    /// Renders every tool result in the turn as text, for evaluators that take a grounding
    /// context.
    /// </summary>
    ...
    /// reading, so it admits anything the model could have read off a tool and rejects only
    /// numbers that appear nowhere in the turn. A stricter version would have to model each
    /// tool's payload, and would start failing when a tool's shape changes rather than when the
