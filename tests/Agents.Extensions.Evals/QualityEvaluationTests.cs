...
/// <remarks>
/// <para>
/// These are measurements, not tests. A judge is a model, so its scores move between runs; a
/// single red metric here is a prompt to look, not proof of a regression. What makes them worth
/// running is that the questions they answer cannot be asserted — whether an answer is coherent,
/// whether it stayed inside what the tools returned, whether the tool call actually served the
/// request.
/// </para>
/// <para>
/// Set <c>EVAL_LIVE_MODEL=1</c> with Ollama running to enable. Judge responses are cached, so
/// re-running an unchanged scenario costs nothing.
/// </para>
/// </remarks>
public sealed class QualityEvaluationTests(ITestOutputHelper output)
{
    ...
    /// <summary>
    /// Grades each answer on relevance, coherence, groundedness and tool use, against the floor
    /// in <see cref="EvalEnvironment.QualityFloor"/>.
    /// </summary>
    [Fact]
    public async Task Answers_ClearTheQualityFloor()
    {
        ...
        foreach (var scenario in WeatherScenarios.Grounded)
        {
            ...
            // Tool Call Accuracy reports an error rather than a score when the response contains
            // no tool call, so say plainly what happened before the evaluator's diagnostic does.
            Assert.True(
    ...
    /// </summary>
    /// <remarks>
    /// Relevance and coherence ask whether an answer is good; equivalence asks whether it is the
    /// right one. It needs a ground truth, which most evaluation suites do not have — this one
    /// does, because the tools return fixed readings, so the reference answer is correct rather
    ...
    public async Task Answers_MatchTheReferenceAnswer()
    {
        ...
        foreach (var scenario in WeatherScenarios.Grounded)
        {
            ...
            var result = await scenarioRun.EvaluateAsync(
                messages,
                response,
                additionalContext: [new EquivalenceEvaluatorContext(scenario.References[0])],
                cancellationToken: cancellationToken);
