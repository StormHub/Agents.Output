...
/// </para>
/// <para>
/// What these tests measure is the harness, not the model — the model's turns are scripted. A
/// green run here means the contexts reach the evaluators, the metrics carry interpretations, and
/// the results land in the store; so when the judged tier goes red, it is the model's fault and
/// not the wiring's.
/// </para>
/// </remarks>
public sealed class OfflineEvaluationTests(ITestOutputHelper output)
{
    ...
    public async Task GroundedAnswers_ScoreAgainstTheirReferences()
    {
        ...
        foreach (var scenario in WeatherScenarios.Grounded)
        {
            ...
            // matches one reference exactly and these scores are 1.0 by construction. The
            // assertion is that the contexts arrived and the scores were computed — the numbers
            // only start carrying information in the live tier, where the answer is not the
            // reference.
    ...
    /// <summary>
    /// The control. A grounding check that cannot fail is asserting nothing, so this run has to
    /// go red.
    /// </summary>
    [Fact]
    public async Task InventedReadings_FailTheGroundingMetric()
    {
        ...
        // The overlap scores discriminate too: this answer is measured against the answer the
        // tools would have supported, and shares little with it.
        Assert.True(result.Get<NumericMetric>(BLEUEvaluator.BLEUMetricName).Value < 0.9);
    }
    ...
    public async Task Results_LandInTheReportingStore()
    {
        ...
        // The conversation is stored with the result, which is what makes a report readable
        // months later: the score and the turn that produced it sit together.
        Assert.NotEmpty(persisted.Messages);
        Assert.Contains(persisted.ModelResponse.Messages, message => message.Contents.Count > 0);

        output.WriteLine($"Read back {persisted.ScenarioName} / {persisted.IterationName} from {EvalEnvironment.StorageRoot}");
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        return await scenarioRun.EvaluateAsync(
            ...
            [
                ...
                new GLEUEvaluatorContext(scenario.References),
