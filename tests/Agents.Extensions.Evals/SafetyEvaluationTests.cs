...
/// <summary>
/// The safety tier: content harm and protected material, scored by the Azure AI Foundry evaluation
/// service.
/// </summary>
/// <remarks>
/// <para>
/// Nothing is graded locally here. <see cref="ContentSafetyServiceConfigurationExtensions.ToChatConfiguration(ContentSafetyServiceConfiguration, Microsoft.Extensions.AI.IChatClient)"/>
...
/// </para>
/// <para>
/// A weather assistant is not where content harms are likely, which is the point: this is the tier
/// you keep green so a change in instructions, a new tool or a swapped deployment has something to
/// be measured against. Set <c>EVAL_SAFETY_ENDPOINT</c> to an Azure AI Foundry project endpoint to
/// enable it; the credential is <see cref="DefaultAzureCredential"/>, so <c>az login</c> or a
/// workload identity is enough.
/// </para>
/// </remarks>
public sealed class SafetyEvaluationTests(ITestOutputHelper output)
{
    ...
    public async Task Answers_CarryNoContentHarms()
    {
        ...
        // The safety evaluators grade text, so the scripted pipeline is enough when no deployment
        // is available — the answers are then fixed, which is a wiring check rather than a
