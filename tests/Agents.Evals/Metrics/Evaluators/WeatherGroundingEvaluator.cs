...
/// <remarks>
/// <para>
/// This is the domain rule the library cannot ship: inventing a temperature is the specific failure
/// this pipeline exists to avoid, and no general-purpose evaluator knows that. Writing it as an
/// <see cref="IEvaluator"/> rather than as a bare assertion is what buys the rest of the machinery
/// — it composes with the shipped evaluators, its metric lands in the same
/// <see cref="EvaluationResult"/>, and the report treats it like any other.
/// </para>
/// <para>
...
internal sealed class WeatherGroundingEvaluator : IEvaluator
{
    ...
    /// <summary>
    /// How far a claimed value may sit from a tool value and still count as grounded. Models round
    /// — "22°C" for a reading of 22.4 is reporting, not inventing.
    /// </summary>
    private const double Tolerance = 0.5;

    /// <summary>
    /// A number is treated as a weather claim only when a unit follows it, so "a 7-day forecast" is
    /// prose and "22.4°C" is a reading.
    /// </summary>
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // Recording what the check compared against is what makes a red metric diagnosable from the
        // report alone, without re-running the scenario.
        metric.AddOrUpdateContext(
            new ToolReadingsContext(
    ...
    /// <summary>
    /// The tool readings the answer was checked against, recorded on the metric.
    /// </summary>
    /// <remarks>
    /// <see cref="EvaluationContext"/> is abstract, so carrying context requires a derived type
    /// even when — as here — it is a plain string. Unlike the contexts the shipped evaluators
    /// take, this one is an output rather than an input: it is attached to the metric after the
    /// check has run, so the report can show what the answer was compared against.
    /// </remarks>
    private sealed class ToolReadingsContext(string readings)
        : EvaluationContext(name: ToolReadingsContextName, content: readings);
}
