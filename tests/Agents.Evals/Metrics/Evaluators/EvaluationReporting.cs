...
namespace Agents.Evals.Metrics.Evaluators;
...
/// <remarks>
/// <para>
/// A <see cref="ReportingConfiguration"/> is the reusable half of the suite: which evaluators run,
/// where results are stored, whether judge responses are cached, and how a raw score becomes a pass
/// or a failure. A <see cref="ScenarioRun"/> — the per-case half — is created from it, and persists
/// the result when it is disposed.
/// </para>
/// <para>
/// All four tiers share one store and one execution name, so a single <c>dotnet aieval report</c>
/// shows the offline checks, the judged scores and the safety severities of the same run side by
/// side.
/// </para>
/// </remarks>
...
{
    /// <summary>
    /// The deterministic tier: reference-overlap scores and the domain grounding rule. No model is
    /// involved, so there is no <see cref="ChatConfiguration"/> and nothing to cache.
    /// </summary>
    ...
    /// <summary>
    /// The judged tier: an LLM grades the answer. Response caching is on, so re-running a red suite
    /// re-reads the judge's verdicts instead of paying for them again.
    /// </summary>
    /// <remarks>
    /// The judge deployment is a caching key. Without it, switching judges would silently serve the
    ...
    /// <see cref="ScenarioRun"/> plumbing carries both.
    /// </summary>
    ...
    public static void Report(ITestOutputHelper output, string scenarioName, EvaluationResult result)
    {
        ...
        foreach (var metric in result.Metrics.Values)
        {
            ...
            output.WriteLine($"  {metric.Name,-28} {value,-10} {verdict}");
    ...
    /// </summary>
    /// <remarks>
    /// A judge that timed out or returned unparseable JSON leaves a diagnostic and a metric with no
    /// value — which is not a failing score, and would otherwise pass silently.
    /// </remarks>
    public static void AssertNoDiagnosticErrors(EvaluationResult result)
    {
        var errors = result.Metrics.Values
            .SelectMany(metric => metric.Diagnostics ?? [])
    ...
    /// <summary>
    /// Applies this suite's pass mark to the judged metrics, and leaves every other metric with the
    /// interpretation its own evaluator produced.
    /// </summary>
    /// <remarks>
    /// This hook is where a suite states its policy. The Quality library defaults to failing
    /// anything under 4.0 out of 5, which a modest deployment rarely clears — and a suite that is
    /// always red is a suite nobody reads. Returning <see langword="null"/> for everything else
    ...
    private static readonly HashSet<string> JudgedMetricNames = new(StringComparer.Ordinal)
    {
        ...
        CompletenessEvaluator.CompletenessMetricName,
