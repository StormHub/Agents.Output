...
public static class WeatherScenarios
{
    ...
    public static readonly WeatherScenario Tokyo = new(
        ...
        [
            new ScriptedToolCall(
                StubWeatherTools.WeatherToolName,
    ...
    public static readonly WeatherScenario Paris = new(
        ...
        ToolCalls:
        [
            new ScriptedToolCall(StubWeatherTools.CalendarToolName),
            new ScriptedToolCall(
                StubWeatherTools.WeatherToolName,
                new Dictionary<string, object?>
                {
    ...
    /// </summary>
    /// <remarks>
    /// A grounding check that never fails is asserting nothing, so this scenario exists to make
    /// it fail. Its reference is the answer the tools would have supported, which is also what
    /// pushes its BLEU, GLEU and F1 well below the grounded scenarios.
    /// </remarks>
